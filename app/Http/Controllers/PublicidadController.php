...
class PublicidadController extends Controller
{
    ...
    {
        $parametros = $parametros = Input::only('status','q','page','per_page', 'id_resolucion');

        $registro = Publicidad::whereNull("deleted_at");
        ...
        if ($parametros['id_resolucion']) {
            $registro =  $registro->where('id_resolucion_administrativa', $parametros['id_resolucion']);
        }
        if ($parametros['q']) {
            $registro =  $registro->where(function($query) use ($parametros) {
                    $query->where('usuario_responsable','LIKE',"%".$parametros['q']."%");
                });
        }
    ...
    public function store(Request $request)
    {
        ...
        $reglas = [
            "no_informe_verificacion"=> 'required',
            "id_medio_utilizado"=> 'required', 
            "usuario_responsable"=> 'required',
            "id_dictamen"=> 'required',
            "id_medida_seguridad"=> 'required',
            "id_resolucion_administrativa"=> 'required',
            "id_sancion_administrativa"=> 'required',
        ...
        */
        $parametros['usuario_responsable'] = strtoupper($parametros['usuario_responsable']);
        //$parametros['observaciones'] = strtoupper($parametros['observaciones']);
        ...
        try{
            DB::beginTransaction();
            $registro = Publicidad::create($parametros);
            DB::commit();
            return Response::json([ 'data' => $registro ],200);
