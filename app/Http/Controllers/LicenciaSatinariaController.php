...
use App\Models\LicenciasSanitarias;

use Illuminate\Support\Facades\Input;
use \Validator,\Hash, \Response, DB;

class LicenciaSatinariaController extends Controller
{
    public function index(Request $request)
    {
        $parametros = $parametros = Input::only('status','q','page','per_page', 'id_dictamen', 'id_giro');

        $registro = LicenciasSanitarias::with("estatus", "giro");
        ...
        if ($parametros['id_dictamen']) {
            $registro =  $registro->where('id_dictamen', $parametros['id_dictamen']);
        }
    ...
    public function show($id)
    {
        try{
            DB::beginTransaction();
            $registro = LicenciasSanitarias::find($id);
            DB::commit();
            return Response::json([ 'data' => $registro ],200);
    ...
    public function store(Request $request)
    {
        ...
        $reglas = [
            ...
            "id_verificacion_sanitaria"=> 'required',
        ...
        */
        ...
        try{
            DB::beginTransaction();
            $registro = LicenciasSanitarias::create($parametros);
            DB::commit();
            return Response::json([ 'data' => $registro ],200);
